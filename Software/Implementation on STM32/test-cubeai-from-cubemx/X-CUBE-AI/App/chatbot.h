...
#ifndef __AI_CHATBOT_H__
#define __AI_CHATBOT_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_CHATBOT_MODEL_NAME          "chatbot"

#define AI_CHATBOT_IN_NUM       (1)
#define AI_CHATBOT_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 51, 1, NULL), \
}
#define AI_CHATBOT_IN_SIZE { \
  (1 * 1 * 51), \
}
#define AI_CHATBOT_IN_1_SIZE  (1 * 1 * 51)
#define AI_CHATBOT_IN_1_SIZE_BYTES  ((1 * 1 * 51) * 4)




#define AI_CHATBOT_OUT_NUM      (1)
#define AI_CHATBOT_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 6, 1, NULL), \
}
#define AI_CHATBOT_OUT_SIZE { \
  (1 * 1 * 6), \
}
#define AI_CHATBOT_OUT_1_SIZE  (1 * 1 * 6)
#define AI_CHATBOT_OUT_1_SIZE_BYTES  ((1 * 1 * 6) * 4)

#define AI_CHATBOT_N_NODES (6)
 ...
 * @ref ai_chatbot_create. Both the activations memory buffer 
 ...
 * using @ref ai_chatbot_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_CHATBOT_*
 * defines into file @ref chatbot.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_chatbot_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_CHATBOT_H__*/
